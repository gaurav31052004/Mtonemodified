...
function generateWebConfig(routes) {
  const routePattern = routes.length > 0 ? routes.join('|') : 'admin'; // fallback
  ...
  <system.webServer>
    <rewrite>
      <rules>
        ...
        <!-- Fallback to root index.html for SPA-like behavior -->
...
function webConfigGeneratorPlugin() {
  return {
    ...
    writeBundle() {
      ...
      console.log(`📁 File location: ${distPath}`);
    }
  };
}
